...
export class SheetEngine {
      ...
      const regions = def.regions.map(region => {
        if (region.type === 'grid' && region.layout) {
          ...
          const hotspots = Geometry.generateGridHotspots(
            region.layout,
            region.layout.type === 'grid' ? ['number', 'checkbox'] : [] // Default, can be overridden
          );
  ...
  private syncHotspotReferences(): void {
    ...
    // Clear all hotspot references first
    for (const region of sheet.definition.regions) {
      region.hotspots?.forEach(h => {
        h.currentMark = undefined;
      });
    }

    // Restore references from marks map
